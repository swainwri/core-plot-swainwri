...
-(void)renderAsVectorInContext:(nonnull CGContextRef)context
{
    ...
    for ( CPTLegendEntry *legendEntry in self.legendEntries ) {
        ...
        if (((desiredRowCount == 0) || (row < desiredRowCount)) &&
            ...
            if( entryPlot == nil ) {
                entryIndex = legendEntry.indexCustomised; // S.Wainwright
                entryPlot  = legendEntry.plotCustomised; // S.Wainwright
            }
            ...
            if ( theLineStyle && legendEntry.plotCustomised == nil ) { // S.Wainwright
            ...
            if ( legendShouldDrawSwatch ) {
                if ( legendEntry.plotCustomised ) { // added S.Wainwright 19/03/21
                    [legendEntry.plotCustomised drawSwatchForLegend:self atIndex:legendEntry.indexCustomised inRect:swatchRect inContext:context];
                }
                else if ( entryPlot ) {  // added S.Wainwright 19/03/21
                    [entryPlot drawSwatchForLegend:self atIndex:entryIndex inRect:swatchRect inContext:context];
                }
                else {
                    CPTFill *theSwatchFill = nil;

                    if ( [theDelegate respondsToSelector:@selector(legend:fillForSwatchAtIndex:forPlot:)] ) {
                        theSwatchFill = [theDelegate legend:self fillForSwatchAtIndex:legendEntry.indexCustomised forPlot:nil];
                    }
                    if ( !theSwatchFill ) {
                        theSwatchFill = self.swatchFill;
                    }

                    CPTLineStyle *theSwatchLineStyle = nil;

                    if ( [theDelegate respondsToSelector:@selector(legend:lineStyleForSwatchAtIndex:forPlot:)] ) {
                        theSwatchLineStyle = [theDelegate legend:self lineStyleForSwatchAtIndex:legendEntry.indexCustomised forPlot:nil];
                    }
                    if ( !theSwatchLineStyle ) {
                        theSwatchLineStyle = self.swatchBorderLineStyle;
                    }

                    if ( theSwatchFill || theSwatchLineStyle ) {
                        CGFloat radius = self.swatchCornerRadius;

                        if ( theSwatchFill ) {
                            CGContextBeginPath(context);
                            CPTAddRoundedRectPath(context, CPTAlignIntegralRectToUserSpace(context, swatchRect), radius);
                        ...
                        if ( theSwatchLineStyle ) {
                            ...
                            CGContextBeginPath(context);
                            CPTAddRoundedRectPath(context, CPTAlignBorderedRectToUserSpace(context, swatchRect, theSwatchLineStyle), radius);
    ...
    CPTGraph *theGraph = self.graph;  // added SWainwright 11/03/2023
    if ( theGraph == nil && self.plots.count ) {
        theGraph = [[self.plots firstObject] graph];
    }
    if ( theGraph.allowTracking ) {
        [self updateLegendTrackingArea];
    }
}

-(void)updateLegendTrackingArea {
    CPTGraph *theGraph = self.graph;  // added SWAinwright 11/03/2023
    if ( theGraph == nil && self.plots.count ) {
        theGraph = [[self.plots firstObject] graph];
    }
#if TARGET_OS_OSX
    NSView *view = (NSView*)theGraph.hostingView;
    if ( theGraph.allowTracking ) {
        NSUInteger index = [view.trackingAreas indexOfObjectPassingTest:^BOOL(NSTrackingArea *area, NSUInteger __unused idx, BOOL * __unused stop) {
            NSDictionary *dict = (NSDictionary*)area.userInfo;
            return [(NSString*)[dict objectForKey:@"annotation"] isEqual:@"CPTLayerAnnotation"] && [(NSString*)[dict objectForKey:@"type"] isEqual:@"legend"];
        }];
        if ( index != NSNotFound ) {
            NSTrackingArea *trackingView = [view.trackingAreas objectAtIndex:index];
            [view removeTrackingArea: trackingView];
        }
        CGRect trackingRect = CGRectMake(self.frame.origin.x, self.frame.origin.y, self.frame.size.width, self.frame.size.height);
        NSTrackingArea *trackingArea = [[NSTrackingArea alloc] initWithRect:trackingRect options:NSTrackingActiveAlways|NSTrackingMouseEnteredAndExited owner:self userInfo:@{ @"message" : @"Main Legend to the graph", @"annotation" : @"CPTLayerAnnotation", @"type": @"legend" }];
        [view addTrackingArea:trackingArea];
    }
#else
    if ( theGraph.allowTracking ) {
        UIView *view = (UIView*)theGraph.hostingView;
...
//        CGRect trackingRect = [self convertRect:self.frame toLayer:view.layer];
        CGRect trackingRect = CGRectMake(self.frame.origin.x, view.frame.size.height - self.frame.origin.y - view.frame.origin.y, self.frame.size.width, self.frame.size.height);
        UIPointerRegion *pointerRegion = [UIPointerRegion regionWithRect:trackingRect identifier:@{ @"message" : @"Main Legend to the graph", @"annotation" : @"CPTLayerAnnotation", @"type": @"legend" }];
...
- (void)mouseEntered:(NSEvent *)event {
    ...
    if ( index != NSNotFound ) {
        ...
        if ( [(NSString*)[(NSDictionary*)[trackingArea userInfo] objectForKey:@"annotation"] isEqualToString:@"CPTLayerAnnotation"] ) {
            theGraph.hoverAnnotation = [[CPTAnnotation alloc] init];
        }
        else {
            return;
        }
