...
#import "CPTPlotSpaceAnnotation.h"  // added S.Wainwright 09/03/2023
#import "CPTMutableLineStyle.h"     // added S.Wainwright 09/03/2023
#import "CPTShadow.h"
...
-(void)updateAxisLabelsAtLocations:(nullable CPTNumberSet *)locations inRange:(nullable CPTPlotRange *)labeledRange useMajorAxisLabels:(BOOL)useMajorAxisLabels
{
    ...
    if ( theGraph.allowTracking ) {
        ...
        NSIndexSet *axisTrackingAreasIndices = [view.trackingAreas indexesOfObjectsPassingTest:^BOOL(NSTrackingArea *area, NSUInteger __unused idx, BOOL * __unused stop) {
            return [(CPTAxis*)[area.userInfo objectForKey:@"axis"] isEqual:self];
        }];
        ...
        for ( CPTAxisLabel *label in newAxisLabels ) {
            ...
            CGRect trackingRect = CGRectMake(origin.x, origin.y, label.contentLayer.frame.size.width, label.contentLayer.frame.size.width);
            NSTrackingArea *trackingArea;
            if ( self.attributedTitle ) {
                NSAttributedString *theAttributedString = self.attributedTitle;
                NSMutableAttributedString *message = [[NSMutableAttributedString alloc] initWithString:@" Axis Label\nValue: "];
                [message insertAttributedString:theAttributedString atIndex:0];
                NSString *theString = ((CPTTextLayer*)label.contentLayer).text;
                NSAttributedString *attributedStringValue = [[NSAttributedString alloc] initWithString:theString];
                [message appendAttributedString:attributedStringValue];
                trackingArea = [[NSTrackingArea alloc] initWithRect:trackingRect options:NSTrackingActiveAlways|NSTrackingMouseEnteredAndExited owner:self userInfo:@{ @"message" : message, @"type" : @"axislabel", @"axis": self, @"plotspace": thePlotSpace, @"axislabel": label }];
            }
            else if ( self.title ) {
                NSString *message = [NSString stringWithFormat:@"%@ Axis Label\nValue: %@", self.title, ((CPTTextLayer*)label.contentLayer).text];
                trackingArea = [[NSTrackingArea alloc] initWithRect:trackingRect options:NSTrackingActiveAlways|NSTrackingMouseEnteredAndExited owner:self userInfo:@{ @"message" : message, @"type" : @"axislabel", @"axis": self, @"plotspace": thePlotSpace, @"axislabel": label }];
            }
            else {
                NSString *message = [NSString stringWithFormat:@"Axis Label\nValue: %@", ((CPTTextLayer*)label.contentLayer).text];
                trackingArea = [[NSTrackingArea alloc] initWithRect:trackingRect options:NSTrackingActiveAlways|NSTrackingMouseEnteredAndExited owner:self userInfo:@{ @"message" : message, @"type" : @"axislabel", @"axis": self, @"plotspace": thePlotSpace, @"axislabel": label  }];
            }
        ...
        for ( CPTAxisLabel *label in newAxisLabels ) {
            ...
            if ( self.attributedTitle ) {
                ...
                NSAttributedString *attributedStringValue = [[NSAttributedString alloc] initWithString:theString];
                [message appendAttributedString:attributedStringValue];
                pointerRegion = [UIPointerRegion regionWithRect:trackingRect identifier:@{ @"message" : message, @"type" : @"axislabel", @"axis": self, @"plotspace": thePlotSpace, @"axislabel": label  }];
            }
            else if ( self.title ) {
                NSString *message = [NSString stringWithFormat:@"%@ Axis Label\nValue: %@", self.title, ((CPTTextLayer*)label.contentLayer).text];
                pointerRegion = [UIPointerRegion regionWithRect:trackingRect identifier:@{ @"message" : message, @"type" : @"axislabel", @"axis": self, @"plotspace": thePlotSpace, @"axislabel": label  }];
...
-(void)updateAxisTitle
{
    ...
    CPTGraph *theGraph = thePlotSpace.graph;
    if ( theGraph.allowTracking ) {
        CPTAxisTitle *theAxisTitle = self.axisTitle;
#if TARGET_OS_OSX
        CPTGraphHostingView *theGraphHostingView = theGraph.hostingView;
        NSView *view = (NSView*)theGraphHostingView;
        NSUInteger index = [view.trackingAreas indexOfObjectPassingTest:^BOOL(NSTrackingArea *area, NSUInteger __unused idx, BOOL * __unused stop) {
            return [(CPTAxis*)[area.userInfo objectForKey:@"axis"] isEqual:self] && [(NSString*)[area.userInfo objectForKey:@"axistitle"] isEqual:self.axisTitle];
        }];
        if ( index != NSNotFound ) {
            [view removeTrackingArea:[view.trackingAreas objectAtIndex:index]];
        }
        
        CGPoint origin = [self viewPointForCoordinateValue:self.titleLocation];
        CGRect trackingRect = CGRectMake(origin.x, origin.y, theAxisTitle.contentLayer.frame.size.width, theAxisTitle.contentLayer.frame.size.width);
        NSTrackingArea *trackingArea;
        if ( self.attributedTitle ) {
            NSAttributedString *theAttributedString = self.attributedTitle;
            NSMutableAttributedString *message = [[NSMutableAttributedString alloc] initWithString:@"Axis Title\n"];
            [message appendAttributedString:theAttributedString];
            trackingArea = [[NSTrackingArea alloc] initWithRect:trackingRect options:NSTrackingActiveAlways|NSTrackingMouseEnteredAndExited owner:self userInfo:@{ @"message" : message, @"type" : @"axistitle", @"axis": self, @"plotspace": thePlotSpace, @"axistitle": theAxisTitle }];
        }
        else if ( self.title ) {
            NSString *message = [NSString stringWithFormat:@"Axis Title\n%@", self.title];
            trackingArea = [[NSTrackingArea alloc] initWithRect:trackingRect options:NSTrackingActiveAlways|NSTrackingMouseEnteredAndExited owner:self userInfo:@{ @"message" : message, @"type" : @"axistitle", @"axis": self, @"plotspace": thePlotSpace, @"axistitle": theAxisTitle }];
        }
        else {
            return;
        ...
        NSUInteger index = [theGraph.pointerRegions indexOfObjectPassingTest:^BOOL(UIPointerRegion *area, NSUInteger __unused idx, BOOL * __unused stop) {
            return [(CPTAxis*)[(NSDictionary*)area.identifier objectForKey:@"axis"] isEqual:self] && [(CPTAxisTitle*)[(NSDictionary*)area.identifier objectForKey:@"axistitle"] isEqual:self.axisTitle];
        }];
        if (index != NSNotFound) {
            [theGraph.pointerRegions removeObjectAtIndex:index];
        }
        
        CGPoint origin = [self viewPointForCoordinateValue:theAxisTitle.tickLocation];
        CGRect trackingRect = CGRectMake(origin.x, ((UIView*)theGraphHostingView).frame.size.height - origin.y - ((UIView*)theGraphHostingView).frame.origin.y, theAxisTitle.contentLayer.frame.size.width, theAxisTitle.contentLayer.frame.size.width);
        UIPointerRegion *pointerRegion;
        if ( self.attributedTitle ) {
            NSAttributedString *theAttributedString = self.attributedTitle;
            NSMutableAttributedString *message = [[NSMutableAttributedString alloc] initWithString:@"Axis Title\n"];
            [message appendAttributedString:theAttributedString];
            pointerRegion = [UIPointerRegion regionWithRect:trackingRect identifier:@{ @"message" : message, @"type" : @"axistitle", @"axis": self, @"plotspace": thePlotSpace, @"axistitle": theAxisTitle }];
        }
        else if ( self.title ) {
            NSString *message = [NSString stringWithFormat:@"Axis Title\n%@", self.title];
            pointerRegion = [UIPointerRegion regionWithRect:trackingRect identifier:@{ @"message" : message, @"type" : @"axistitle", @"axis": self, @"plotspace": thePlotSpace, @"axistitle": theAxisTitle }];
...
- (void)mouseEntered:(NSEvent *)event {
    ...
    if ( [theGraph getHoverAnnotation] ) {
        [theGraph removeAnnotation:[theGraph getHoverAnnotation]];
        [theGraph setHoverAnnotation:nil];
    }
    
    NSView *theHostingView = (NSView*)theGraph.hostingView;
    ...
    NSUInteger index = [theHostingView.trackingAreas indexOfObjectPassingTest:^BOOL(NSTrackingArea *area, NSUInteger __unused idx, BOOL * __unused stop) {
        ...
        return [addr isEqualToString:trackNum] /*&& [(NSString*)[area.userInfo objectForKey:@"type"] isEqualToString:@"axislabel"]*/;
    }];
    if ( index != NSNotFound ) {
        ...
        if ( self == (CPTAxis*)[(NSDictionary*)[trackingArea userInfo] objectForKey:@"axis"] ) {
            CPTPlotSpace *thePlotSpace = self.plotSpace;
            ...
            [theGraph setHoverAnnotation: annotation];
        }
        else {
            return;
        }
        
        CPTAnnotation *theHoverAnnotation = [theGraph getHoverAnnotation];
        id<CPTGraphHoverDelegate> theHoverDelegate = (id<CPTGraphHoverDelegate>)theGraph.hoverDelegate;
        CPTLayer *layer;
        
        if ( [theHoverDelegate respondsToSelector:@selector(hoverLabelForGraph:trackingArea:)] ) {
            layer = [theHoverDelegate hoverLabelForGraph:theGraph trackingArea:trackingArea];
        }
        else {
