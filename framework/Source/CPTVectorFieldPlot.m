...
CPTVectorFieldPlotBinding const CPTVectorFieldPlotBindingPlotSymbols = @"plotSymbols"; ///< Plot symbols.
...
/** @property CPTFieldDataSourceBlock  dataSourceBlockX
 *  @brief block to supply vector field with function evaluator.
 **/
@synthesize dataSourceBlockX;

/** @property CPTFieldDataSourceBlock  dataSourceBlockY
 *  @brief block to supply vector field with function evaluator.
 **/
...
+(void)initialize
{
    if ( self == [CPTVectorFieldPlot class] ) {
        ...
        [self exposeBinding:CPTVectorFieldPlotBindingPlotSymbols];
    }
}
    ...
    if ( self.doublePrecisionCache ) {
        ...
        dispatch_apply(dataCount, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(size_t i) {
            ...
            if ( !drawPointFlags[i] || isnan(x) || isnan(y) ) {
                ...
                plotPoint[CPTCoordinateX] = x + length * sin(direction);
                plotPoint[CPTCoordinateY] = y + length * cos(direction);
...
-(void)reloadPlotSymbolsInIndexRange:(NSRange)indexRange
{
    ...
    if ( [theDataSource respondsToSelector:@selector(symbolsForVectorFieldPlot:recordIndexRange:)] ) {
        needsLegendUpdate = YES;

        [self cacheArray:[theDataSource symbolsForVectorFieldPlot:self recordIndexRange:indexRange]
                  forKey:CPTVectorFieldPlotBindingPlotSymbols
           atRecordIndex:indexRange.location];
    }
    else if ( [theDataSource respondsToSelector:@selector(symbolForVectorFieldPlot:recordIndex:)] ) {
        needsLegendUpdate = YES;
        ...
        for ( NSUInteger idx = indexRange.location; idx < maxIndex; idx++ ) {
            CPTPlotSymbol *symbol = [theDataSource symbolForVectorFieldPlot:self recordIndex:idx];
            if ( symbol ) {
        ...
        [self cacheArray:array forKey:CPTVectorFieldPlotBindingPlotSymbols atRecordIndex:indexRange.location];
    }
...
{
    CPTPlotSymbol *symbol = [self cachedValueForKey:CPTVectorFieldPlotBindingPlotSymbols recordIndex:idx];
...
-(void)reloadVectorLineStylesInIndexRange:(NSRange)indexRange
{
    ...
    if ([theDataSource isKindOfClass:[CPTFieldFunctionDataSource class]]) {
        theDataSource = (id<CPTVectorFieldPlotDataSource>)self.appearanceDataSource;
    }
            ...
            if ( self.useFastRendering ) {
                ...
                for ( NSUInteger i = (NSUInteger)firstDrawnPointIndex; i <= (NSUInteger)lastDrawnPointIndex; i++ ) {
                    if ( drawPointFlags[i] ) {
                        ...
                        if ( [currentSymbol isKindOfClass:symbolClass] ) {
                            [currentSymbol renderInContext:context atPoint:CGPointMake(viewPoints[i].x, viewPoints[i].y) scale:scale alignToPixels:alignPoints];
                        }
                    }
                }
            }
                ...
                for ( NSUInteger i = (NSUInteger)firstDrawnPointIndex; i <= (NSUInteger)lastDrawnPointIndex; i++ ) {
                    if ( drawPointFlags[i] ) {
                        CPTPlotSymbol *currentSymbol = [self plotSymbolForRecordIndex:i];
                        if ( [currentSymbol isKindOfClass:symbolClass] ) {
                            [currentSymbol renderAsVectorInContext:context atPoint:CGPointMake(viewPoints[i].x, viewPoints[i].y) scale:(CGFloat)1.0];
                        }
                    }
                }
            }
    ...
    if ( result != NSNotFound ) {
        ...
        for ( NSUInteger i = (NSUInteger)result; i < dataCount; ++i ) {
            if ( drawPointFlags[i] ) {
                ...
                CGFloat distanceSquared = squareOfDistanceBetweenPoints(viewPoint, lastPoint);
        ...
        if ( result != NSNotFound ) {
            ...
            if ( !isnan(lastViewPoint.tip_x) && (viewPoint.x > lastViewPoint.tip_x) ) {
                result = NSNotFound;
            }
            if ( !isnan(lastViewPoint.tip_y) && (viewPoint.x > lastViewPoint.tip_y) ) {
                result = NSNotFound;
            }
        }
    }
...
//    NSParameterAssert(idx < self.cachedDataCount);
    ...
    if ( self.doublePrecisionCache ) {
        double plotPoint[2];
        plotPoint[CPTVectorFieldPlotFieldX] = [self cachedDoubleForField:CPTVectorFieldPlotFieldX recordIndex:idx];
        plotPoint[CPTVectorFieldPlotFieldY] = [self cachedDoubleForField:CPTVectorFieldPlotFieldY recordIndex:idx];

        viewPoint = [thePlotSpace plotAreaViewPointForDoublePrecisionPlotPoint:plotPoint numberOfCoordinates:2];
    }
    else {
        NSDecimal plotPoint[2];
        plotPoint[CPTVectorFieldPlotFieldX] = [self cachedDecimalForField:CPTVectorFieldPlotFieldX recordIndex:idx];
        plotPoint[CPTVectorFieldPlotFieldY] = [self cachedDecimalForField:CPTVectorFieldPlotFieldY recordIndex:idx];
...
-(BOOL)pointingDeviceDownEvent:(nonnull CPTNativeEvent *)event atPoint:(CGPoint)interactionPoint
{
    ...
    if ( [theDelegate respondsToSelector:@selector(vectorFieldPlot:plotSymbolTouchDownAtRecordIndex:)] ||
         [theDelegate respondsToSelector:@selector(vectorFieldPlot:plotSymbolTouchDownAtRecordIndex:withEvent:)] ||
         [theDelegate respondsToSelector:@selector(vectorFieldPlot:plotSymbolWasSelectedAtRecordIndex:)] ||
         [theDelegate respondsToSelector:@selector(vectorFieldPlot:plotSymbolWasSelectedAtRecordIndex:withEvent:)] ) {
        ...
        if ( idx != NSNotFound ) {
            ...
            if ( CGRectContainsPoint(symbolRect, plotAreaPoint)) {
                ...
                if ( [theDelegate respondsToSelector:@selector(vectorFieldPlot:plotSymbolTouchDownAtRecordIndex:)] ) {
                    ...
                    [theDelegate vectorFieldPlot:self plotSymbolTouchDownAtRecordIndex:idx];
                }
                if ( [theDelegate respondsToSelector:@selector(vectorFieldPlot:plotSymbolTouchDownAtRecordIndex:withEvent:)] ) {
                    symbolTouchUpHandled = YES;
                    [theDelegate vectorFieldPlot:self plotSymbolTouchDownAtRecordIndex:idx withEvent:event];
                }
            }
        }
    }
...
-(BOOL)pointingDeviceUpEvent:(nonnull CPTNativeEvent *)event atPoint:(CGPoint)interactionPoint
{
    ...
    if ( [theDelegate respondsToSelector:@selector(vectorFieldPlot:plotSymbolTouchUpAtRecordIndex:)] ||
         [theDelegate respondsToSelector:@selector(vectorFieldPlot:plotSymbolTouchUpAtRecordIndex:withEvent:)] ||
         [theDelegate respondsToSelector:@selector(vectorFieldPlot:plotSymbolWasSelectedAtRecordIndex:)] ||
         [theDelegate respondsToSelector:@selector(vectorFieldPlot:plotSymbolWasSelectedAtRecordIndex:withEvent:)] ) {
        // Inform delegate if a point was hit
        CGPoint plotAreaPoint = [theGraph convertPoint:interactionPoint toLayer:thePlotArea];
        ...
        if ( idx != NSNotFound ) {
            ...
            if ( CGRectContainsPoint(symbolRect, plotAreaPoint)) {
                ...
                if ( [theDelegate respondsToSelector:@selector(vectorFieldPlot:plotSymbolTouchUpAtRecordIndex:)] ) {
                    ...
                    [theDelegate vectorFieldPlot:self plotSymbolTouchUpAtRecordIndex:idx];
                }
                if ( [theDelegate respondsToSelector:@selector(vectorFieldPlot:plotSymbolTouchUpAtRecordIndex:withEvent:)] ) {
                    symbolSelectHandled = YES;
                    [theDelegate vectorFieldPlot:self plotSymbolTouchUpAtRecordIndex:idx withEvent:event];
                }

                if ( idx == selectedDownIndex ) {
                    if ( [theDelegate respondsToSelector:@selector(vectorFieldPlot:plotSymbolWasSelectedAtRecordIndex:)] ) {
                        symbolSelectHandled = YES;
                        [theDelegate vectorFieldPlot:self plotSymbolWasSelectedAtRecordIndex:idx];
                    }

                    if ( [theDelegate respondsToSelector:@selector(vectorFieldPlot:plotSymbolWasSelectedAtRecordIndex:withEvent:)] ) {
                        symbolSelectHandled = YES;
                        [theDelegate vectorFieldPlot:self plotSymbolWasSelectedAtRecordIndex:idx withEvent:event];
                    }
                }
            }
        }
