...
@property (nonatomic, readwrite, strong, nullable) CPTAnnotation *_hoverAnnotation;  // SWainwright 07/03/2023
@property (nonatomic, readwrite, assign) BOOL inTitleUpdate;

#if TARGET_OS_IOS || TARGET_OS_SIMULATOR || TARGET_OS_IPHONE || TARGET_OS_MACCATALYST
@property (nonatomic, readwrite, strong, nullable) UIHoverGestureRecognizer *hoverGesture;   // SWainwright 09/03/2023
@property (nonatomic, readwrite, strong, nullable) UIPointerInteraction *pointerInteraction;    // SWainwright 09/03/2023
#endif
...
/** @property nullable id<CPTGraphHoverDelegate> hoverDelegate
 *  @brief The hover delegate for the graph.
 **/
@synthesize hoverDelegate;
...
@synthesize _hoverAnnotation;
@synthesize inTitleUpdate;

#if TARGET_OS_IOS || TARGET_OS_SIMULATOR || TARGET_OS_IPHONE || TARGET_OS_MACCATALYST
@synthesize hoverGesture;   // SWainwright 09/03/2023
@synthesize pointerInteraction;    // SWainwright 09/03/2023
#endif

/** @property BOOL allowTracking
 *  @brief Whether tracking of mouse over Annotations is allowed. Default is @NO.
 **/
@synthesize allowTracking; // added S.Wainwright 07/03/2023

#if TARGET_OS_IOS
/** @property NSMutableArray<UIPointerRegion*> pointerRegions
 *  @brief Mutable Array to store pointerRegions  Default is @null.
 **/
@synthesize pointerRegions;
#endif
...
-(nonnull instancetype)initWithLayer:(nonnull id)layer
{
    if ((self = [super initWithLayer:layer])) {
        ...
        hoverDelegate            = theLayer->hoverDelegate;
...
#if TARGET_OS_IOS
    [self.pointerRegions removeAllObjects];
#endif
}
...
// added S.Wainwright
/** @brief Retrieve the legend Layer Annotation of the graph.
 **/
-(nullable CPTLayerAnnotation *)getLegendLayerAnnotation {
    return  self.legendAnnotation;
}

#pragma mark -
#pragma mark Hover Annotation

// added S.Wainwright
/** @brief Retrieve the hover Annotation of the graph.
 **/
-(nullable CPTAnnotation *)getHoverAnnotation {
    return self._hoverAnnotation;
}

-(void)setHoverAnnotation:(nullable CPTAnnotation *)newAnnotation {
    self._hoverAnnotation = newAnnotation;
}
...
-(void)setAllowTracking:(BOOL)newAllowTracking {
    
#if TARGET_OS_OSX
    NSView *view = self.hostingView;
...
//            CGRect trackingRect = [self convertRect:self.titleAnnotation.contentLayer.frame toLayer:view.layer];
        ...
        CPTAnnotationArray *plotAreaAnnotations = self.plotAreaFrame.plotArea.annotations;
        if ( plotAreaAnnotations.count ) {
            for ( CPTLayerAnnotation *annotation in plotAreaAnnotations ) {
                CGRect trackingRect = [self convertRect:annotation.contentLayer.frame toLayer:view.layer];
                NSTrackingArea *trackingArea = [[NSTrackingArea alloc] initWithRect:trackingRect options:NSTrackingActiveAlways|NSTrackingMouseEnteredAndExited owner:self userInfo:@{ @"message" : @"Layer Annotation", @"annotation" : @"CPTLayerAnnotation", @"type": @"plotAreaAnnotation"} ];
                [view addTrackingArea:trackingArea];
            }
        }
    ...
    if ( self.allowTracking ) {
        ...
        UIView *view = self.hostingView;
        [view addInteraction:thePointerInteraction];
    
        if ( self.titleAnnotation ) {
            CGRect trackingRect = [self convertRect:self.titleAnnotation.contentLayer.frame toLayer:view.layer];
            trackingRect = CGRectMake(trackingRect.origin.x, trackingRect.origin.y, trackingRect.origin.y, trackingRect.size.height);
            UIPointerRegion *pointerRegion = [UIPointerRegion regionWithRect:trackingRect identifier:@{ @"message" : @"Title to the graph", @"annotation" : @"CPTLayerAnnotation", @"type": @"plotAreaAnnotation" }];
        ...
        if ( plotAreaAnnotations.count ) {
            for ( CPTLayerAnnotation *annotation in plotAreaAnnotations ) {
                CGRect trackingRect = [self convertRect:annotation.contentLayer.frame toLayer:view.layer];
                trackingRect = CGRectMake(trackingRect.origin.x, trackingRect.origin.y, trackingRect.size.width, trackingRect.size.height);
...
//-(void)updateTrackingAreas { // no need to do this as these annotations are fixed in one place
//    
//    if ( self.allowTracking ) {
//        CALayer *superlayer = self.superlayer;
//#if TARGET_OS_OSX
//        CPTGraphHostingView *theGraphHostingView = self.hostingView;
//        NSView *view = (NSView*)theGraphHostingView;
//        NSUInteger index = [view.trackingAreas indexOfObjectPassingTest:^BOOL(NSTrackingArea *area, NSUInteger __unused idx, BOOL * __unused stop) {
//            NSDictionary *dict = area.userInfo;
//            return [(NSString*)[dict objectForKey:@"annotation"] isEqualToString:@"CPTLayerAnnotation"] && [(NSString*)[dict objectForKey:@"type"] isEqualToString:@"title"];
//        }];
//        if ( index != NSNotFound ) {
//            CGRect trackingRect = [self convertRect:self.titleAnnotation.contentLayer.frame toLayer:superlayer];
//            NSTrackingArea *oldTrackingArea = [view.trackingAreas objectAtIndex:index];
//            NSTrackingArea *newTrackingArea = [[NSTrackingArea alloc] initWithRect:trackingRect options:oldTrackingArea.options owner:self userInfo:oldTrackingArea.userInfo];
//            [view removeTrackingArea:oldTrackingArea];
//            [view addTrackingArea:newTrackingArea];
//        }
//        index = [view.trackingAreas indexOfObjectPassingTest:^BOOL(NSTrackingArea *area, NSUInteger __unused idx, BOOL * __unused stop) {
//            NSDictionary *dict = area.userInfo;
//            return [(NSString*)[dict objectForKey:@"annotation"] isEqualToString:@"CPTLayerAnnotation"] && [(NSString*)[dict objectForKey:@"type"] isEqualToString:@"legend"];
//        }];
//        if ( index != NSNotFound ) {
//            CGRect trackingRect = [self convertRect:self.legendAnnotation.contentLayer.frame toLayer:superlayer];
//            NSTrackingArea *oldTrackingArea = [view.trackingAreas objectAtIndex:index];
//            NSTrackingArea *newTrackingArea = [[NSTrackingArea alloc] initWithRect:trackingRect options:oldTrackingArea.options owner:self userInfo:oldTrackingArea.userInfo];
//            [view removeTrackingArea:oldTrackingArea];
//            [view addTrackingArea:newTrackingArea];
//        }
//#else
//        NSUInteger index =  = [theGraph.pointerRegions indexOfObjectPassingTest:^BOOL(UIPointerRegion *area, NSUInteger __unused idx, BOOL * __unused stop) {
//            NSDictionary *dict = (NSDictionary*)area.identifier;
//            return [(NSString*)[dict objectForKey:@"annotation"] isEqualToString:@"CPTLayerAnnotation" && [(NSString*)[dict objectForKey:@"type"] isEqualToString:@"title"];
//        }];
//        if ( index != NSNotFound ) {
//            UIPointerRegion *pointerRegion = [self.pointerRegions objectAtIndex:index];
//            pointerRegion.rect = [self convertRect:self.titleAnnotation.contentLayer.frame toLayer:superlayer];
//        }
//        index =  = [theGraph.pointerRegions indexOfObjectPassingTest:^BOOL(UIPointerRegion *area, NSUInteger __unused idx, BOOL * __unused stop) {
//            NSDictionary *dict = (NSDictionary*)area.identifier;
//            return [(NSString*)[dict objectForKey:@"annotation"] isEqualToString:@"CPTLayerAnnotation" && [(NSString*)[dict objectForKey:@"type"] isEqualToString:@"legend"];
//        }];
//        if ( index != NSNotFound ) {
//            UIPointerRegion *pointerRegion = [self.pointerRegions objectAtIndex:index];
//            pointerRegion.rect = [self convertRect:self.legendAnnotation.contentLayer.frame toLayer:superlayer];
//        }
//#endif
//    }
//}
...
-(UIPointerRegion*)pointerInteraction:(UIPointerInteraction *)interaction regionForRequest:(UIPointerRegionRequest *)request defaultRegion:(UIPointerRegion *)defaultRegion {
    NSMutableArray<UIPointerRegion*> *thePointerRegions = self.pointerRegions;
    UIPointerRegion *activeRegion = nil;
    for ( UIPointerRegion *pointerRegion in thePointerRegions) {
        if ( CGRectContainsPoint(pointerRegion.rect, request.location) ) {
            activeRegion = pointerRegion;
            break;
        }
    }
    return activeRegion ? activeRegion : defaultRegion;
}
...
-(void)pointerInteraction:(UIPointerInteraction *)interaction willEnterRegion:(UIPointerRegion *)region animator:(id<UIPointerInteractionAnimating>)animator {
    CPTGraphHostingView *theGraphHostingView = self.hostingView;
    if ( !CGRectEqualToRect(theGraphHostingView.frame, region.rect) ) {
        // remove previous hover annotation
        if ( self._hoverAnnotation ) {
            [self removeAnnotation:self._hoverAnnotation];
            self._hoverAnnotation = nil;
        }
        
        NSUInteger index = [self.pointerRegions indexOfObjectPassingTest:^BOOL(UIPointerRegion *area, NSUInteger __unused idx, BOOL * __unused stop) {
            return [area isEqual:region];
        }];
        if ( index != NSNotFound ) {
            CPTPlotSpace *thePlotSpace = nil;
            UIPointerRegion *pointerRegion = [self.pointerRegions objectAtIndex:index];
            CGPoint adjustedPosition = CGPointMake(pointerRegion.rect.origin.x + pointerRegion.rect.size.width / 2, theGraphHostingView.frame.size.height - pointerRegion.rect.origin.y - pointerRegion.rect.size.height / 2);
            if ( [(NSString*)[(NSDictionary*)[pointerRegion identifier] objectForKey:@"annotation"] isEqualToString:@"CPTLayerAnnotation"] ) {
                self.hoverAnnotation = [[CPTAnnotation alloc] init];
            }
            else if ( (thePlotSpace = (CPTPlotSpace*)[(NSDictionary*)[pointerRegion identifier] objectForKey:@"plotspace"]) ) {
                CPTNumberArray *plotPoint = [thePlotSpace plotPointForPlotAreaViewPoint:adjustedPosition];
                self.hoverAnnotation = [[CPTPlotSpaceAnnotation alloc] initWithPlotSpace:thePlotSpace anchorPlotPoint:plotPoint];
            }
            else {
                return;
            }
            
            CPTAnnotation *theHoverAnnotation = self._hoverAnnotation;
            
            id<CPTGraphHoverDelegate> theHoverDelegate = (id<CPTGraphHoverDelegate>)self.hoverDelegate;
            CPTLayer *layer;
            
            if ( [theHoverDelegate respondsToSelector:@selector(hoverLabelForGraph:pointerRegion:)] ) {
                layer = [theHoverDelegate hoverLabelForGraph:self pointerRegion:pointerRegion];
            }
            else {
                CPTTextLayer *textLayer = [[CPTTextLayer alloc] initWithText:(NSString*)[(NSDictionary*)[pointerRegion identifier] objectForKey:@"message"]];
                ...
                lineStyleBorder.lineColor = [CPTColor grayColor];
...
-(void)pointerInteraction:(UIPointerInteraction *)interaction willExitRegion:(UIPointerRegion *)region animator:(id<UIPointerInteractionAnimating>)animator {
    CPTGraphHostingView *theGrapHostingView = self.hostingView;
    if ( !CGRectEqualToRect(theGrapHostingView.frame, region.rect) ) {
        // remove previous hover annotation
        if ( self._hoverAnnotation ) {
            [self removeAnnotation:self._hoverAnnotation];
            self._hoverAnnotation = nil;
...
- (void)mouseEntered:(NSEvent *)event {
    ...
    CPTGraphHostingView *theHostingView = self.hostingView;
    NSUInteger index = [theHostingView.trackingAreas indexOfObjectPassingTest:^BOOL(NSTrackingArea *area, NSUInteger __unused idx, BOOL * __unused stop) {
        NSString *addr = [NSString stringWithFormat:@"%p", area];
        NSString *trackNum = [NSString stringWithFormat:@"0x%0lx", event.trackingNumber];
        return [addr isEqualToString:trackNum];
    }];
    if ( index != NSNotFound ) {
//        CPTPlotSpace *thePlotSpace = nil;
        NSTrackingArea *trackingArea = [theHostingView.trackingAreas objectAtIndex:index];
        if ( [(NSString*)[(NSDictionary*)[trackingArea userInfo] objectForKey:@"annotation"] isEqualToString:@"CPTLayerAnnotation"] ) {
            self.hoverAnnotation = [[CPTAnnotation alloc] init];
        }
//        else if ( (thePlotSpace = (CPTPlotSpace*)[(NSDictionary*)[trackingArea userInfo] objectForKey:@"plotspace"]) ) {
...
//        }
        ...
        if ( [theHoverDelegate respondsToSelector:@selector(hoverLabelForGraph:trackingArea:)] ) {
            layer = [theHoverDelegate hoverLabelForGraph:self trackingArea:trackingArea];
        }
        else {
            CPTTextLayer *textLayer = [[CPTTextLayer alloc] initWithText:(NSString*)[(NSDictionary*)[trackingArea userInfo] objectForKey:@"message"]];
            CPTColor *fillColour = [CPTColor colorWithComponentRed:1.0 green:1.0 blue:0.762 alpha:0.6];
            textLayer.fill = [CPTFill fillWithColor: fillColour];
            textLayer.paddingTop = 5;
            textLayer.paddingLeft = 5;
