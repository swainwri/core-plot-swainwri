...
-(void)renderAsVectorInContext:(nonnull CGContextRef)context {
    ...
    if ( graph ) {
        ratio = (double)(graph.bounds.size.width / graph.bounds.size.height);
    }
    ...
    if ( firstDrawnPointIndex != NSNotFound && lastDrawnPointIndex != NSNotFound ) {
        ...
        if ( !self.extrapolateToLimits && !self.functionPlot ) {
            ...
            if (thePlotSpace.xRange.lengthDouble > thePlotSpace.yRange.lengthDouble) {
                if ([self.limits[0] doubleValue] == -DBL_MAX && [self.limits[1] doubleValue] == DBL_MAX) {
                    ...
                    limit3 = thePlotSpace.yRange.minLimitDouble + thePlotSpace.xRange.lengthDouble / ratio < [self.limits[3] doubleValue] ? thePlotSpace.yRange.minLimitDouble + thePlotSpace.xRange.lengthDouble / ratio : [self.limits[3] doubleValue];
                }
            }
            else {
                if ([self.limits[2] doubleValue] == -DBL_MAX && [self.limits[3] doubleValue] == DBL_MAX) {
                    ...
                    limit0 = thePlotSpace.xRange.minLimitDouble > [self.limits[0] doubleValue] ? thePlotSpace.xRange.minLimitDouble : [self.limits[0] doubleValue];
                    limit1 = thePlotSpace.xRange.minLimitDouble + thePlotSpace.yRange.lengthDouble * ratio < [self.limits[1] doubleValue] ? thePlotSpace.xRange.minLimitDouble + thePlotSpace.yRange.lengthDouble * ratio : [self.limits[1] doubleValue];
                }
            }
        }
        ...
        NSUInteger repeatContoursCalculation = 2;
        ...
        CGFloat currentMaxWidthPixels = self.maxWidthPixels, currentMaxHeightPixels = self.maxHeightPixels;
        ...
        if ( self.needsIsoCurvesUpdate ) {
            ...
            while ( repeatContoursCalculation > 0 ) {
                ...
                if ( !self.extrapolateToLimits && !self.functionPlot && !CGRectEqualToRect(self.greatestContourBox, CGRectZero) && (CGRectEqualToRect(self.greatestContourBox, previousGreatestContourBox) || CGRectContainsRect(self.greatestContourBox, previousGreatestContourBox)) ) {
                    ...
                    contours = [[CPTContours alloc] initWithNoIsoCurve:self.noActualIsoCurves IsoCurveValues:planesValues Limits:_limits];
                    self.extrapolateToLimits = YES;
                }
                else {
...
//                currentContext = (CGContextRef)bitmapContext.CGContext;
                ...
                plotPoint0[CPTCoordinateX] = limit0;
                plotPoint0[CPTCoordinateY] = limit2;
                plotPoint1[CPTCoordinateX] = limit1;
                plotPoint1[CPTCoordinateY] = limit3;
...
//                for ( NSUInteger iPlane = 0; iPlane < self.isoCurvesIndices.count; iPlane++ ) {
////                    if ( !(/*iPlane == 0 || iPlane == 1 ||*/ iPlane >= 3) ) continue;
//                    plane = [[self.isoCurvesIndices objectAtIndex:iPlane] unsignedIntegerValue];
////    #if DEBUG
...
//                                    if ( (stripContours.array[0].x == leftEdge || stripContours.array[0].x == rightEdge ||  stripContours.array[0].y == bottomEdge || stripContours.array[0].y == topEdge) && (stripContours.array[stripContours.used - 1].x == leftEdge || stripContours.array[stripContours.used - 1].x == rightEdge || stripContours.array[stripContours.used - 1].y == bottomEdge || stripContours.array[stripContours.used - 1].y == topEdge) ) {
//                                        ;
//                                    }
//                                    else {
...
-(void) drawFillBetweenClosedIsoCurves:(nonnull CGContextRef)context contours:(CPTContours *)contours usedExtraLineStripLists:(BOOL*)usedExtraLineStripLists leftEdge:(CGFloat)leftEdge bottomEdge:(CGFloat)bottomEdge rightEdge:(CGFloat)rightEdge topEdge:(CGFloat)topEdge {
    ...
    for ( NSInteger i = processInAscendingOrder ? 0 : (NSInteger)self.isoCurvesIndices.count - 1 ; processInAscendingOrder ? i < (NSInteger)self.isoCurvesIndices.count : i > -1; processInAscendingOrder ? i++ : i-- ) {
        actualPlane = [[self.isoCurvesIndices objectAtIndex:(NSUInteger)i] unsignedIntegerValue];
                    if ( !(actualPlane <= 3 /*|| actualPlane == 3 || actualPlane == 5*/) ) continue;
        ...
        for( NSUInteger j = 0; j < (NSUInteger)closedStrips.used; j++ ) {
            NSUInteger noAdjustIsoCurvesThisStrip = 1; // since a contour can overlap another need to break this down and 'refDataLinePath'
            // looks at each overlapped path
            ...
            if ( !CGPathIsEmpty(refDataLinePath) ) {
                ...
                for(NSUInteger k = 0; k < (NSUInteger)closedStrips.used; k++) {
                    ...
                    if ( CGPathContainsPoint(refDataLinePath, &transform, closedStrips.array[k].startPoint, YES) ) {
                        ...
                        //                        collectedPlanes[countCollectedPlanes] = actualPlane;
                        //                        countCollectedPlanes++;
                    }
                }
                CGContextSaveGState(context);
                
                CGMutablePathRef *overlappedPaths = NULL;
//                NSUInteger noOverlappedPaths = 0, biggestOverlappedPathIndex = NSNotFound;
                plane = (NSUInteger)i;
                noFoundDataLinePaths = [self findClosedDataLinePaths:&foundDataLinePaths noFoundClosedDataLinePaths:noFoundDataLinePaths foundClosedPlanes:&foundPlanes OuterCGPath:&refDataLinePath context:context contours:contours plane:plane leftEdge:leftEdge bottomEdge:bottomEdge rightEdge:rightEdge topEdge:topEdge ascendingOrder:processInAscendingOrder useExtraLineStripList:(BOOL)closedStrips.array[j].extra fromCurrentPlane:NO checkPointOnPath:NO];
//                if ( (noFoundDataLinePaths = [self findClosedDataLinePaths:&foundDataLinePaths noFoundClosedDataLinePaths:noFoundDataLinePaths foundClosedPlanes:&foundPlanes OuterCGPath:&refDataLinePath context:context contours:contours plane:plane leftEdge:leftEdge bottomEdge:bottomEdge rightEdge:rightEdge topEdge:topEdge ascendingOrder:processInAscendingOrder useExtraLineStripList:(BOOL)closedStrips.array[j].extra fromCurrentPlane:NO checkPointOnPath:NO]) > 0 ) {
//                    for ( NSUInteger l = 0; l < noFoundDataLinePaths; l++ ) {
//                        // see if the inner contour overlaps refDataLinePath
//                        if ( [self createCGPathFrom2OverlappingPathsWithCommonEdge:refDataLinePath aPath2:foundDataLinePaths[l] overlapPaths:&overlappedPaths noOverlapPaths:&noOverlappedPaths] ) {
//
//                            CGMutablePathRef *combinedOverlapDataLinePaths = (CGMutablePathRef*)calloc((size_t)noOverlappedPaths, sizeof(CGMutablePathRef));
//                            biggestOverlappedPathIndex = [self findLargestOverlappedPath:context contours:contours overlappedPaths:overlappedPaths noOverlappedPaths:noOverlappedPaths collectedPlanes:collectedPlanes combinedOverlapDataLinePaths:&combinedOverlapDataLinePaths leftEdge:leftEdge bottomEdge:bottomEdge rightEdge:rightEdge topEdge:topEdge ascendingOrder:processInAscendingOrder useExtraLineStripList:(BOOL)closedStrips.array[j].extra];
//                            free(combinedOverlapDataLinePaths);
//                            noAdjustIsoCurvesThisStrip = noOverlappedPaths;
//                        }
//                    }
//                }
                ...
                BOOL areOverlappedPaths = overlappedPaths != NULL && !CGPathIsEmpty(overlappedPaths[0]);
                CGMutablePathRef outerCheckValueCGPath = CGPathCreateMutable(), innerCheckValueCGPath = CGPathCreateMutable();
                for( NSUInteger k = 0; k < noAdjustIsoCurvesThisStrip; k++) {
                    if ( areOverlappedPaths ) {
                        CGPathRelease(refDataLinePath);
                        refDataLinePath = CGPathCreateMutableCopy(overlappedPaths[k]);
                        CGPathRelease(overlappedPaths[k]);
//                        if ( k == biggestOverlappedPathIndex ) {
//                            CGPathAddPath(outerCheckValueCGPath, &transform, refDataLinePath);
//                        }
                    }
                    ...
                    if ( !CGPathIsEmpty(refDataLinePath) ) {
                        ...
                        if ( areOverlappedPaths ) {
                            ...
                            NSUInteger *combinedPlanes = (NSUInteger*)calloc(1, sizeof(NSUInteger));
                            ...
                            noCombinedFoundDataLinePaths = [self findClosedDataLinePaths:&combinedDataLinePaths noFoundClosedDataLinePaths:noCombinedFoundDataLinePaths foundClosedPlanes:&combinedPlanes OuterCGPath:&combinedDataLinePath context:context contours:contours plane:plane leftEdge:leftEdge bottomEdge:bottomEdge rightEdge:rightEdge topEdge:topEdge ascendingOrder:processInAscendingOrder useExtraLineStripList:(BOOL)closedStrips.array[j].extra fromCurrentPlane:YES checkPointOnPath:YES];
                            noCombinedFoundDataLinePaths = [self findClosedDataLinePaths:&combinedDataLinePaths noFoundClosedDataLinePaths:noCombinedFoundDataLinePaths foundClosedPlanes:&combinedPlanes OuterCGPath:&combinedDataLinePath context:context contours:contours plane:plane leftEdge:leftEdge bottomEdge:bottomEdge rightEdge:rightEdge topEdge:topEdge ascendingOrder:!processInAscendingOrder useExtraLineStripList:(BOOL)closedStrips.array[j].extra fromCurrentPlane:YES checkPointOnPath:YES];
                            for( NSUInteger m = 0; m < noCombinedFoundDataLinePaths; m++ ) {
                                if( !self.extrapolateToLimits && !self.functionPlot ) {
                                    CGPathCloseSubpath(combinedDataLinePaths[m]);
                                }
                                if ( CGPathEqualToPath(combinedDataLinePaths[m], refDataLinePath) ) {
                                    continue;
                                }
#if DEBUG
#if TARGET_OS_OSX
                                [bezierPath appendBezierPath:[NSBezierPath bezierPathWithCGPath:combinedDataLinePaths[m]]];
#else
                                [bezierPath appendPath:[UIBezierPath bezierPathWithCGPath:combinedDataLinePaths[m]]];
#endif
#endif
                                CGContextAddPath(context, combinedDataLinePaths[m]);
//                                if ( k == biggestOverlappedPathIndex ) {
//                                    CGPathAddPath(innerCheckValueCGPath, &transform, combinedDataLinePaths[m]);
//                                }
                                CGPathRelease(combinedDataLinePaths[m]);
                                collectedPlanes = (NSUInteger*)realloc(collectedPlanes, sizeof(NSUInteger) * (size_t)(countCollectedPlanes + 1));
                                collectedPlanes[countCollectedPlanes] = combinedPlanes[m];
                                countCollectedPlanes++;
                            }
...
//                            CGPathAddPath(outerCheckValueCGPath, &transform, refDataLinePath);
                            for( NSUInteger l = 0; l < noFoundDataLinePaths; l++ ) {
                                if( !self.extrapolateToLimits && !self.functionPlot ) {
                                    CGPathCloseSubpath(foundDataLinePaths[l]);
                                }
#if DEBUG
#if TARGET_OS_OSX
                                NSBezierPath *bezierPath1 = [NSBezierPath bezierPathWithCGPath:foundDataLinePaths[l]];
                                [bezierPath appendBezierPath:bezierPath1];
#else
                                UIBezierPath *bezierPath1 = [UIBezierPath bezierPathWithCGPath:foundDataLinePaths[l]];
                                [bezierPath appendPath:bezierPath1];
#endif
                                CGContextAddPath(context, foundDataLinePaths[l]);
//                                CGPathAddPath(innerCheckValueCGPath, &transform, foundDataLinePaths[l]);
#endif
                            }
                        }
                    }
                ...
                free(foundPlanes);
                if ( overlappedPaths != NULL ) {
                    free(overlappedPaths);
                }
                ...
                if ( countCollectedPlanes == 1 ) {
                    ...
                    if ( CGPathIsEmpty(innerCheckValueCGPath) ) {
#if DEBUG
#if TARGET_OS_OSX
                        NSBezierPath * __unused bezierPath = [NSBezierPath  bezierPathWithCGPath:outerCheckValueCGPath];
#else
                        UIBezierPath * __unused bezierPath = [UIBezierPath  bezierPathWithCGPath:outerCheckValueCGPath];
#endif
#endif
                        theFill = [self findFillFromBoundingPlanes:collectedPlanes noCollectedPlanes:countCollectedPlanes];
                        //                        countCollectedPlanes = 1;
...
#if DEBUG
#if TARGET_OS_OSX
                        NSBezierPath *bezierPath = [NSBezierPath  bezierPathWithCGPath:outerCheckValueCGPath];
                        NSBezierPath *bezierPath1 = [NSBezierPath  bezierPathWithCGPath:innerCheckValueCGPath];
                        [bezierPath appendBezierPath:bezierPath1];
#else
                        UIBezierPath *bezierPath = [UIBezierPath  bezierPathWithCGPath:outerCheckValueCGPath];
                        UIBezierPath *bezierPath1 = [UIBezierPath  bezierPathWithCGPath:innerCheckValueCGPath];
                        [bezierPath appendPath:bezierPath1];
#endif
#endif
                        theFill = [self findFillFromBoundingPlanes:collectedPlanes noCollectedPlanes:countCollectedPlanes];
                ...
                CGPathRelease(outerCheckValueCGPath);
                CGPathRelease(innerCheckValueCGPath);
        ...
        while ( borderIndex < borderIndices->used ) {
            ...
            while ( YES ) {
                ...
                if ( borderStrips->array[index].plane == NSNotFound ) { // is a discontinuity border already have CGPath
                    ...
                    startPoint = borderStrips->array[index].startPoint;
                    endPoint = borderStrips->array[index].endPoint;
                }
                else {
    ...
    if ( (ascendingOrder && plane + 1 < [self.isoCurvesIndices count]) || (!ascendingOrder && (NSInteger)plane - 1 > -1) ) {
        ...
        if( [self checkForClosedIsoCurvesInsideOuterIsoCurve:context contours:contours Plane:currentPlane Strips:&workingStrips ascendingOrder:ascendingOrder useExtraLineStripList:useExtraLineStripList] ) {
            ...
            for( NSUInteger i = 0; i < (NSUInteger)workingStrips.used; i++ ) {
                ...
                if( CGPathContainsPoint(*outerCGPath, &transform, centreOfPath, YES) ) {
                    foundCGPath = YES;
#if TARGET_OS_OSX
...
#pragma mark Clustering
        ...
        //        gmmCluster.init_num_of_subclasses = 20;
        [gmmCluster cluster];
...
//#if DEBUG
//                      [symbol renderAsVectorInContext:context atPoint:discontinuities[m] scale:(CGFloat)1.0];
//#endif
                        m++;
                    }
                }
                ...
                for( NSUInteger j = i + 1; j < noClusters; j++ ) {
                    ...
                    for ( NSUInteger k = 0; k < clustersOuterNoPoints[j]; k++ ) {
                        ...
                        while ( /*(pos =*/ kdtree_iterator_get_next(result)/*)*/ != KDTREE_END ) {
                            //                            double distance = sqrt( ((double)clustersOuterPoints[j][k].x-x[pos])*((double)clustersOuterPoints[j][k].x-x[pos]) +
                            //                                                   ((double)clustersOuterPoints[j][k].y-y[pos])*((double)clustersOuterPoints[j][k].y-y[pos]) );
                            //                            printf("distance: %f cluster %ld to cluster %ld\n", distance , i , j);
                            foundOne = YES;
                            break;
                        }
                        if (foundOne) {
...
//              [symbol renderAsVectorInContext:context atPoint:clustersOuterPoints[i][k] scale:(CGFloat)1.0];
//           }
//#endif
            // redo concavity on larger value

            [hull setConcavity:200.0];
            [hull concaveHullOnViewPoints:clustersOuterPoints[i] dataCount:clustersOuterNoPoints[i]];
...
//            clustersOuterPoints[i][[hull hullpointsCount]] = CGPointMake([hull hullpointsArray][0].point.x, [hull hullpointsArray][0].point.y);
        ...
        for( NSUInteger i = 0; i < noClusters; i++) {
            ...
            *(*boundaryLimitsDataLinePaths + i) = CGPathCreateMutable();
            ...
            strip.startBorderdirection = [self findPointBorderDirection:strip.startPoint leftEdge:leftEdge bottomEdge:bottomEdge rightEdge:rightEdge topEdge:topEdge];
            ...
            for( NSUInteger j = k + 1; j < clustersOuterNoPoints[i]; j++ ) {
                ...
                else if ( (clustersOuterPoints[i][j].x - clustersOuterPoints[i][j-1].x == 0) || (clustersOuterPoints[i][j].y - clustersOuterPoints[i][j-1].y == 0)  ) {
            ...
            strip.endPoint = CGPathGetCurrentPoint(*boundaryLimitsDataLinePath);
            strip.endBorderdirection = [self findPointBorderDirection:strip.endPoint leftEdge:leftEdge bottomEdge:bottomEdge rightEdge:rightEdge topEdge:topEdge];
