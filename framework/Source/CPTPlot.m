...
#import "CPTMutableLineStyle.h" // S.Wainwright 09/03/2023
#import "CPTColor.h"            // S.Wainwright 09/03/2023
#import <tgmath.h>
...
-(void)updateTrackingAreas {
    ...
    if ( theGraph.allowTracking ) {
        ...
        NSIndexSet *plotTrackingAreasIndices = [view.trackingAreas indexesOfObjectsPassingTest:^BOOL(NSTrackingArea *area, NSUInteger __unused idx, BOOL * __unused stop) {
            ...
            return [(NSString*)[dict objectForKey:@"plotidentifier"] isEqual:(NSString*)self.identifier] && [(NSString*)[dict objectForKey:@"type"] isEqual:@"datalabel"];
        }];
        ...
        for ( CPTAnnotation *annotation in self.labelAnnotations ) {
            if ( ![annotation isKindOfClass: [NSNull class]]) {
                ...
                if ( anchorPlotPoint ) {
                    ...
                    NSString *message = [NSString stringWithFormat:@"Data Point\nValue: %@", ((CPTTextLayer*)plotSpaceAnnotation.contentLayer).text];
                    NSTrackingArea *trackingArea = [[NSTrackingArea alloc] initWithRect:trackingRect options:NSTrackingActiveAlways|NSTrackingMouseEnteredAndExited owner:self userInfo:@{ @"message": message, @"annotation": @"CPTPlotSpaceAnnotation", @"plotspace": thePlotSpace, @"plotidentifier": (NSString*)self.identifier, @"type": @"datalabel" }];
        ...
        for ( CPTAnnotation *annotation in self.labelAnnotations ) {
            if ( ![annotation isKindOfClass: [NSNull class]]) {
                ...
                if ( anchorPlotPoint ) {
                    ...
                    CGRect trackingRect = CGRectMake(origin.x, origin.y, plotSpaceAnnotation.contentLayer.frame.size.width, plotSpaceAnnotation.contentLayer.frame.size.width);
                    NSString *message = [NSString stringWithFormat:@"Data Point\nValue: %@", ((CPTTextLayer*)plotSpaceAnnotation.contentLayer).text];
                    UIPointerRegion *pointerRegion =  [UIPointerRegion regionWithRect:trackingRect identifier:@{ @"message" : message, @"annotation" : @"CPTPlotSpaceAnnotation", @"plotspace": thePlotSpace, @"plotidentifier": (NSString*)self.identifier, @"type": @"datalabel" }];
