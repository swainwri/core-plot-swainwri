...
class HullHelper {
    ...
    func getHull(_ pointSet: [Any], concavity: Double, format: [String]?) -> [Any] {
        ...
        let concave: [HullPoint] = concaveFunc(&convex, pow(concavity, 2), maxSearchArea, grid, &skipList)
    ...
    func concaveFunc(_ convex: inout [HullPoint], _ maxSqEdgeLen: Double, _ maxSearchArea: [Double],
                     _ grid: Grid, _ edgeSkipList: inout [String: Bool]) -> [HullPoint] {
